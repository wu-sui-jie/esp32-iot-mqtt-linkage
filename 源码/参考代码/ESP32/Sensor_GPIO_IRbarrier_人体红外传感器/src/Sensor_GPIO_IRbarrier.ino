...
int mqttstate=0;
bool irBarrierState_last = false;
...
int len = 0;
//发送消息的数据格式
#define JSON_DATA_IRBARRIER "{\"IRstate\":%d}"
...
void mainFunction() {
  // Serial.print("sensortype:");
  // Serial.println(sensortype);
  bool irBarrierState = readIRBarrier();
  Serial.print("IR Barrier State: ");
  Serial.println(irBarrierState ? "BLOCKED" : "CLEAR");
  ShowIRBarrierDashboard(irBarrierState, wifi_con_flag, mqttstate, groupid);
  if (second % 10 == 5 || irBarrierState != irBarrierState_last) {
    irBarrierState_last = irBarrierState;
    cleanBuffer(ATdata, BUF_DATA_LEN);
    len = snprintf(ATdata, BUF_DATA_LEN, JSON_DATA_IRBARRIER, irBarrierState ? 1 : 0);
    Serial.println(ATdata);
    postMsg("Wifi", "IRbarrier", ATdata);
  }
}
...
// 新增红外对射传感器信息显示界面
void ShowIRBarrierDashboard(bool irBarrierState, bool wifi_online, bool mqtt_connected, String group_id) {
  TFT_SetTextColor(WHITE, BLUE);
  TFT_SetFontSize(SONG_STYLE24);
  TFT_ShowString(30, 6, "红外对射");
  ...
  TFT_SetTextColor(irBarrierState ? RED : GREEN, BLACK);
  TFT_ShowString(110, 80, irBarrierState ? "有遮挡" : "无遮挡");
