...
int mqttstate=0;
bool sr602State_last = false;
unsigned long lastNoDetectionTime = 0;  // 上次未检测到人的时间戳
const unsigned long NO_DETECTION_TIMEOUT = 10000;  // 连续10秒未检测到人才判断为无人
...
int len = 0;
//发送消息的数据格式
#define JSON_DATA_SR602 "{\"SR602state\":%d}"
...
void setup() {
  ...
  TFT_ShowString(10, 35, "（人体感应）系统启动中...");
...
void mainFunction() {
  ...
  bool sr602State = false;
  
  // 直接检测传感器状态
  bool currentDetection = readSR602();
  
  if (currentDetection) {
    // 检测到人
    sr602State = true;
    lastNoDetectionTime = 0; // 重置未检测计时器
  } else {
    // 未检测到人
    if (sr602State_last) {
      // 之前状态是有人，现在未检测到人
      ...
      // 检查是否连续10秒未检测到人
      if (currentTime - lastNoDetectionTime >= NO_DETECTION_TIMEOUT) {
        // 连续10秒未检测到人，状态变为无人
        sr602State = false;
        lastNoDetectionTime = 0; // 重置计时器
      } else {
        // 未达到10秒，保持有人状态
        sr602State = true;
      }
  ...
  Serial.print("SR602 State: ");
  Serial.println(sr602State ? "DETECTED" : "NO DETECTION");
  ShowSR602Dashboard(sr602State, wifi_con_flag, mqttstate, groupid);
  
  if (second % 10 == 5 || sr602State != sr602State_last) {
    sr602State_last = sr602State;
    cleanBuffer(ATdata, BUF_DATA_LEN);
    len = snprintf(ATdata, BUF_DATA_LEN, JSON_DATA_SR602, sr602State ? 1 : 0);
    Serial.println(ATdata);
    postMsg("Wifi", "SR602", ATdata);
  }
}
...
// 红外人体感应传感器信息显示界面
void ShowSR602Dashboard(bool sr602State, bool wifi_online, bool mqtt_connected, String group_id) {
  TFT_SetTextColor(WHITE, BLUE);
  TFT_SetFontSize(SONG_STYLE24);
  TFT_ShowString(30, 6, "红外人体感应");
  ...
  TFT_ShowString(30, 80, "状态:");
  TFT_SetTextColor(sr602State ? RED : GREEN, BLACK);
  TFT_ShowString(110, 80, sr602State ? "有人" : "无人");
