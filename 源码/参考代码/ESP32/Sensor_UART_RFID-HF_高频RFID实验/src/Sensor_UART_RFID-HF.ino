...
// JSON数据格式定义
#define JSON_DATA_RFID "{\"HFRFIDCARD\":%s}"

// 卡号转换函数
String convertCardNumber(String originalCard) {
  if (originalCard.length() < 6) {
    return "0000000000"; // 卡号长度不足，返回10个0
  }
  
  // 取前6位
  String first6Digits = originalCard.substring(0, 6);
  
  // 字节序反转：8F792B -> 2B798F
  String reversed = "";
  for (int i = first6Digits.length() - 2; i >= 0; i -= 2) {
    reversed += first6Digits.substring(i, i + 2);
  }
  
  // 转换为10进制
  unsigned long decimalValue = strtoul(reversed.c_str(), NULL, 16);
...
void setup() {
  ...
  // 测试卡号转换功能
  Serial.println("测试卡号转换功能:");
  String testCard = "8F792B83";
  String converted = convertCardNumber(testCard);
  Serial.print("原始卡号: ");
...
void mainFunction() {
  if (sensortype == "01") {
    ...
    if (rfid_card != "") {
      Serial.print("检测到RFID卡: ");
      Serial.println(rfid_card);
      
      // 显示转换过程
      String convertedCard = convertCardNumber(rfid_card);
      Serial.print("转换后卡号: ");
      Serial.println(convertedCard);
    }
    ShowRFIDDashboard(rfid_card, wifi_con_flag, mqttstate, groupid);
    rfid_card_flag = (rfid_card != rfid_card_prev);  //卡号状态发生变化（包括从有卡到无卡）
    if (rfid_card_flag) {
      cleanBuffer(ATdata, BUF_DATA_LEN);
      if (rfid_card != "") {
        // 有卡时发送转换后的卡号
        String convertedCard = convertCardNumber(rfid_card);
        len = snprintf(ATdata, BUF_DATA_LEN, JSON_DATA_RFID, convertedCard.c_str());
        Serial.print("检测到RFID卡变化，发送转换后卡号: ");
      } else {
        // 无卡时发送空值
        len = snprintf(ATdata, BUF_DATA_LEN, JSON_DATA_RFID, "0");
        Serial.print("RFID卡已移除，发送0");
      }
      Serial.println(ATdata);
      postMsg("Wifi", "HFRFID", ATdata);
    }
...
void ShowRFIDDashboard(String card_id, bool wifi_online, bool mqtt_connected, String group_id) {
  ...
  // 显示转换后的卡号
  TFT_SetTextColor(RED, BLACK);
  TFT_ShowString(30, 120, "转换:");
  TFT_ClearArea(100, 120, 120, 32);
  if (card_id == "") {
    TFT_ShowString(100, 120, "无卡");
  } else {
    String convertedCard = convertCardNumber(card_id);
    TFT_ShowString(100, 120, convertedCard.c_str());
  }
