...
void parseMqttResponse(char *payload) {
  ...
  if (error) {
    ...
    Serial.println("commType: " + commType);
    String sensorType = root["sensorType"];
    Serial.println("sensorType: " + sensorType);

    //判断commType
    if (commType == "LoRa") {
      Serial.println("Received LoRa command, sending to Serial2...");
      // 如果是LoRa数据，直接通过串口2发送
      if (root.containsKey("data")) {
        String data = root["data"];
        Serial2.println(data);
        Serial.println("Sent to Serial2: " + data);
      } else if (root.containsKey("cmd")) {
        String cmd = root["cmd"];
        Serial2.println(cmd);
        Serial.println("Sent to Serial2: " + cmd);
      }
    }
    //如果是WiFi
    /*
