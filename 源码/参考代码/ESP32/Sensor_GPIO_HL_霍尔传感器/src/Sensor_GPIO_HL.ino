...
bool hallDetected_last = false;  // 霍尔传感器上次状态
...
int len = 0;
//发送消息的数据格式
#define JSON_DATA_HALL "{\"MAGstate\":%d}"
...
void mainFunction() {
  // Serial.print("sensortype:");
  // Serial.println(sensortype);
  bool hallDetected = readHallDigital();
  Serial.print("Hall Detected: ");
  Serial.println(hallDetected ? "YES" : "NO");
  ShowHallDashboard(hallDetected, wifi_con_flag, mqttstate, groupid);
  if (second % 10 == 5 || hallDetected != hallDetected_last) {
    hallDetected_last = hallDetected;
    cleanBuffer(ATdata, BUF_DATA_LEN);
    len = snprintf(ATdata, BUF_DATA_LEN, JSON_DATA_HALL, hallDetected ? 1 : 0);
    Serial.println(ATdata);
    postMsg("Wifi", "3144", ATdata);
  }
}
...
// 新增霍尔传感器信息显示界面（仅数字量）
void ShowHallDashboard(bool hallDetected, bool wifi_online, bool mqtt_connected, String group_id) {
  TFT_SetTextColor(WHITE, BLUE);
  TFT_SetFontSize(SONG_STYLE24);
  TFT_ShowString(30, 6, "霍尔监测");
  ...
  TFT_ShowString(30, 100, "霍尔状态:");
  // 根据有磁/无磁设置不同颜色
  if (hallDetected) {
    TFT_SetTextColor(RED, BLACK); // 有磁用红色
    TFT_ShowString(135, 100, "有磁");
  } else {
    TFT_SetTextColor(BLUE, BLACK);   // 无磁用蓝色
    TFT_ShowString(135, 100, "无磁");
  }
