// ==========================================================
//  5 号板 · ADC/RGB/蜂鸣器（rgb、buzzer） · 本地联动
//  文件：linkage.cpp
//
//  旁听 report 主题，把其他板的状态变成灯光与声音：
//    8 号 light_sensor 光照过暗  →  RGB 暖白常亮（照明）
//    6 号 flame 检测到火焰       →  RGB 多彩循环 + 蜂鸣器间歇报警
//
//  【两个场景抢同一组灯，所以要有仲裁】
//  优先级：火焰（最高）> 平台手动命令 > 光照照明。
//
//  用"状态重算"而不是"来一条事件改一次灯"：把三个输入都记成状态，
//  任一变化就整体重算一次场景。这样事件以任何顺序到达，
//  最终状态都一致，也不会留下"火焰灭了灯还红着"这类残影。
// ==========================================================
